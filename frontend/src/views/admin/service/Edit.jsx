...
export default function Edit() {
  ...
  const fetchData = async()=>{
     await axiosClient.get(`service/edit/${id}`).then((res)=>{
       setService(res.data);
       document.getElementById('image_preview_icon').setAttribute('src', `http://localhost:8000/image/service/${res.data.icon}`);
       document.getElementById('image_preview_image').setAttribute('src', `http://localhost:8000/image/service/${res.data.image}`);
    ...
    const handleSubmit = (event)=>{
      ...
      axiosClient.post(`service/update/${service.id}`,data).then((response)=>{
