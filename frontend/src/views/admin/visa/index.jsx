...
export default function index() {
  ...
  const fetchData = async () => {
    await axiosClient.get('visa/index').then((res)=>{
       setVisaList(res.data.visa);
     });
  ...
  const deleteAction = (id) => {
    axiosClient.get(`visa/delete/${id}`).then((res)=>{
    ...
    <div className="flex justify-between overflow-x-auto  sm:rounded-lg bg-gray-100 shadow-xl p-4">
      <div className="w-1/2 bg-white overflow-x-auto  sm:rounded-lg p-8 border-none  rounded-md shadow-sm">
        <div className="flex  justify-between pr-24 ">
          ...
          <Link to='create'>
          <span className='bg-blue-600 h-full p-3 text-white fontstyle font-semibold rounded-md fontstyle text-sm'>Add Visa</span>
          </Link>
        </div>
        <table className="w-full text-sm text-left rtl:text-right text-gray-500  ">
          ...
          <tbody>
            ...
            <td className="px-6 py-4">
                <img src={`http://localhost:8000/image/visas/${item.image}`} className="w-16 md:w-32 max-w-full max-h-full" alt={item.title}/>
            </td>
